...
export interface ReportInsights {
  ...
  output: { completedOutcomeCount: number; workloadPerOutcome: number; outcomes: Outcome[] };
  reminders: string[];
}
...
export function buildReportInsights(current: WorkRecord[], previous: WorkRecord[], outcomes: Outcome[]): ReportInsights {
  ...
  const output = {
    ...
    workloadPerOutcome: completedOutcomes.length ? Number((currentWorkload / completedOutcomes.length).toFixed(1)) : 0,
...
export function buildAutomaticReportSummary(insights: ReportInsights): string {
  ...
  return [
    ...
    `形成 ${insights.output.completedOutcomeCount} 项阶段性或已完成成果。`
  ].join("\n");
}
