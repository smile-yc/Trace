...
export function buildAnnualOutputPackage(
  records: WorkRecord[],
  outcomes: Outcome[],
  workloadAdjustmentPercent = 100
): AnnualOutputPackage {
  ...
  const linkedRecordIds = new Set(
    reportableOutcomes.flatMap((outcome) => outcome.recordIds ?? []).filter((id) => currentById.has(id))
  );
  ...
  const projects = Array.from(projectGroups.entries()).map(([name, projectRecords]) => {
    ...
    const projectOutcomes = outcomes.filter((outcome) =>
      outcome.projectName?.trim() === name || (outcome.recordIds ?? []).some((id) => projectRecordIds.has(id))
    );
    return {
      name,
