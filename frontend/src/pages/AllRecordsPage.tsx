  ...
  return (
    <div className="ledger-page">
      ...
      >
        ...
        {rangePreset === "week" && <input className="ledger-period-control" aria-label="指定周" type="week" value={filters.period} onChange={(event) => updateFilter("period", event.target.value)} />}
        {rangePreset === "month" && <input className="ledger-period-control" aria-label="指定月" type="month" value={filters.period} onChange={(event) => updateFilter("period", event.target.value)} />}
        {rangePreset === "year" && <input className="ledger-period-control" aria-label="指定年" min="2000" max="2100" type="number" value={filters.period} onChange={(event) => updateFilter("period", event.target.value)} />}
        {rangePreset === "custom" && <div className="ledger-custom-range ledger-period-control"><input aria-label="开始日期" type="date" value={filters.startDate} onChange={(event) => updateFilter("startDate", event.target.value)} /><span>至</span><input aria-label="结束日期" type="date" value={filters.endDate} onChange={(event) => updateFilter("endDate", event.target.value)} /></div>}
